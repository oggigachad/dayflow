...
# How far back each named range reaches, inclusive of today.
_SPANS = {"day": 0, "week": 6, "month": 29}
...
def my_attendance(
    ...
    start = _today() - timedelta(days=_SPANS[range])
    return (
        db.query(Attendance)
...
def all_attendance(
    ...
    range: Literal["day", "week", "month"] | None = Query(
        None, description="Trailing window ending today; ignored when 'on' is set"
    ),
    ...
    # 'on' stays a single day for the daily table. 'range' feeds the calendar and
    # weekly views, which need more than one day in one round trip.
    if on is not None or range is None:
        query = query.filter(Attendance.date == (on or _today()))
    else:
        query = query.filter(Attendance.date >= _today() - timedelta(days=_SPANS[range]))
